...
// ---------------------------
// Helper: simple directory creation (non-recursive)
// ---------------------------
static bool ensure_dir(const std::string& dir)
{
#if defined(_WIN32)
...
static bool has_image_extension(const std::string& path)
{
    ...
    std::string ext = to_lower_copy(path.substr(dot));
...
static std::vector<std::string> get_all_images_cvglob(const std::string& base_dir)
{
    ...
    // glob(pattern, result, recursive). pattern "dir/*" is common.
    cv::glob(base_dir + "/*", all, true);
    out.reserve(all.size());
    for (const auto& p : all)
    {
        std::string s = (std::string)p;
        if (has_image_extension(s)) out.push_back(s);
    }
    return out;
}
...
int main(int argc, char** argv)
{
    ...
    {
        std::cerr << "Failed to create output dir: " << out_dir << std::endl;
        return -1;
    }
    ...
    if (images.empty())
    {
        std::cerr << "No images found under: " << test_dir << std::endl;
        return -2;
    }
    ...
    for (size_t idx = 0; idx < images.size(); idx++)
    {
        ...
        if (img.empty())
        {
            std::cerr << "Skip unreadable: " << path << std::endl;
            continue;
        }
        ...
        {
            const face_box& f = faces[fi];
            // clamp for drawing
            int x0 = std::max(0, std::min((int)std::round(f.x0), img.cols - 1));
            int y0 = std::max(0, std::min((int)std::round(f.y0), img.rows - 1));
            int x1 = std::max(0, std::min((int)std::round(f.x1), img.cols - 1));
            int y1 = std::max(0, std::min((int)std::round(f.y1), img.rows - 1));

            // Always log to CSV (including suppressed ones) if you want only drawn, gate by draw_threshold here.
            csv << path << "," << fi << "," << f.score << ","
                << f.x0 << "," << f.y0 << "," << f.x1 << "," << f.y1;

            for (int k = 0; k < LANDMARK_NUMBER; k++)
            {
                csv << "," << f.landmark.x[k] << "," << f.landmark.y[k];
            }
            csv << "\n";
            ...
            for (int k = 0; k < LANDMARK_NUMBER; k++)
            {
                int lx = std::max(0, std::min((int)std::round(f.landmark.x[k]), img.cols - 1));
                int ly = std::max(0, std::min((int)std::round(f.landmark.y[k]), img.rows - 1));
                cv::circle(vis, cv::Point(lx, ly), 2, cv::Scalar(0, 255, 255), -1);
            }
            char buf[64];
    ...
    std::cout << "Done. Results written to: " << out_dir << std::endl;
    return 0;
}
