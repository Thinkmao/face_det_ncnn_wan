...
// -----------------------------------------------------------------------------
// False-positive suppression (precision-oriented)
// These heuristics are not part of "core RetinaFace", but are common in production:
// 1) Box size: penalize boxes that are much smaller than the average detected size
// 2) Aspect ratio: penalize very elongated boxes (often background edges)
// 3) Center density: penalize isolated boxes whose centers have few nearby neighbors
// All penalties are applied BEFORE sorting + NMS, so they influence final picks.
// -----------------------------------------------------------------------------
...
static inline float box_size_sqrt_area(const face_box& b)
{
    ...
    return std::sqrt(w * h); // sqrt(area) in pixels (target_size space)
}

static inline float box_aspect_sym(const face_box& b)
{
    float w = box_w(b);
    float h = box_h(b);
    if (w <= 0.f || h <= 0.f) return 999.f;
    float ar = w / h;
    return ar >= 1.f ? ar : (1.f / ar); // >= 1, distance from square
}

static inline void apply_fp_suppression(face_box* boxes, int& count,
                                        float prob_gate,       // boxes below this score are ignored by heuristics
                                        float keep_threshold,  // final score threshold after penalties
                                        float size_ratio_thr,  // small if size < avg_size * size_ratio_thr
                                        float ar_start,        // start penalizing when aspect_sym > ar_start
                                        float ar_hard,         // heavy penalty when aspect_sym >= ar_hard
                                        float ar_k,            // exp penalty strength
                                        float density_iou_thr, // neighbor iou threshold for density counting
                                        int   density_min_nbr, // desired minimum neighbor count
                                        float density_r_scale, // neighbor radius scale * min(w,h)
                                        float density_bias_px) // neighbor radius extra bias in pixels
{
    if (!boxes || count <= 0) return;

    // ---- (1) Compute average size from boxes that pass prob_gate ----
    float sum_size = 0.f;
    int   n_size = 0;
    ...
    float avg_size = (n_size > 0) ? (sum_size / n_size) : 0.f;
    float small_thr = avg_size * size_ratio_thr;

    // ---- (2) Size + aspect ratio penalties ----
    for (int i = 0; i < count; ++i)
    {
        if (boxes[i].score < prob_gate) continue;
        ...
        {
            float s = box_size_sqrt_area(boxes[i]);
            if (s > 0.f && s < small_thr)
            {
                float factor = clampf(s / small_thr, 0.f, 1.f); // linear
                boxes[i].score *= factor;
            }
        }

        // Aspect ratio penalty (elongated boxes are often false positives)
        float ar_sym = box_aspect_sym(boxes[i]);
        if (ar_sym >= ar_hard)
        {
            boxes[i].score *= 0.05f; // heavy punishment
        }
        else if (ar_sym > ar_start)
        {
            // exp(-k * (ar - ar_start))
            float factor = std::exp(-ar_k * (ar_sym - ar_start));
            boxes[i].score *= clampf(factor, 0.f, 1.f);
        }
    }

    // ---- (3) Center density penalty (isolation) ----
    // Count neighbors whose centers are close AND IoU is non-trivial.
    // True faces usually produce a cluster of nearby candidates (different anchors/scales).
    for (int i = 0; i < count; ++i)
    {
        if (boxes[i].score < prob_gate) continue;

        float wi = box_w(boxes[i]);
        float hi = box_h(boxes[i]);
        if (wi <= 0.f || hi <= 0.f) continue;

        float cxi = 0.5f * (boxes[i].x0 + boxes[i].x1);
        float cyi = 0.5f * (boxes[i].y0 + boxes[i].y1);

        float r = density_r_scale * std::min(wi, hi) + density_bias_px;
        float r2 = r * r;
        ...
        for (int j = 0; j < count; ++j)
        {
            ...
            float cxj = 0.5f * (boxes[j].x0 + boxes[j].x1);
            float cyj = 0.5f * (boxes[j].y0 + boxes[j].y1);

            float dx = cxj - cxi;
            float dy = cyj - cyi;
            if (dx * dx + dy * dy > r2) continue;

            float iou = CalculateOverlap(boxes[i], boxes[j]);
            if (iou < density_iou_thr) continue;
        ...
        {
            // linear penalty: 0 neighbors -> 0.25, (min_nbr-1) -> ~0.625 ...
            float factor = clampf((nbr + 1.f) / (density_min_nbr + 1.f), 0.f, 1.f);
            // make it more aggressive for precision
            factor = 0.25f + 0.75f * factor;
            boxes[i].score *= factor;
        }
    }

    // ---- (4) Filter out low-score boxes after penalties (in-place compaction) ----
    int write = 0;
    for (int i = 0; i < count; ++i)
    {
        if (boxes[i].score >= keep_threshold)
        {
            if (write != i) boxes[write] = boxes[i];
            write++;
        }
    }
    count = write;
...
static int detect_retinaface_forward(const ncnn::Mat& bgr, ncnn::Mat& output, std::vector<face_box>& out_p_faces)
{
    ...
    // Precision-oriented false-positive suppression (size/aspect/density).
    // Applied in target_size (320x320) coordinate space before sort+NMS.
    // Tuned for "prefer fewer false positives" (may increase miss rate on tiny/blur faces).
    int refined_face_count = total_face_count;

    // Parameters (precision-first defaults):
    // - prob_gate: only candidates with score >= this participate in statistics/density checks
    // - keep_threshold: candidates below this after penalties are dropped
    // - size_ratio_thr: "small" if sqrt(area) < avg_size * size_ratio_thr, then linear score decay
    // - ar_start/ar_hard/ar_k: aspect-ratio penalty (ar_sym = max(w/h, h/w))
    // - density_*: isolation penalty based on nearby neighbor boxes
    const float fp_prob_gate      = 0.50f;
    const float fp_keep_threshold = 0.60f;
    const float fp_size_ratio_thr = 0.60f;

    const float fp_ar_start = 1.80f;
    const float fp_ar_hard  = 3.00f;
    const float fp_ar_k     = 1.20f;

    const float fp_density_iou_thr = 0.05f;
    const int   fp_density_min_nbr = 2;
    const float fp_density_r_scale = 0.25f;
    const float fp_density_bias_px = 4.0f;

    apply_fp_suppression(total_face_box, refined_face_count,
                         fp_prob_gate, fp_keep_threshold,
                         fp_size_ratio_thr,
                         fp_ar_start, fp_ar_hard, fp_ar_k,
                         fp_density_iou_thr, fp_density_min_nbr,
                         fp_density_r_scale, fp_density_bias_px);

    total_face_count = refined_face_count;
    if (total_face_count <= 0)
    {
        // No candidates left after suppression.
        if (box_list_32) free(box_list_32);
        if (box_list_16) free(box_list_16);
        if (box_list_8)  free(box_list_8);
        if (total_face_box) free(total_face_box);
        return 0;
    }
...
// �ݹ��ȡ����ͼƬ�ļ�·��
std::vector<fs::path> get_all_images(const fs::path& dir_path) {
	std::vector<fs::path> image_files;

	if (!fs::exists(dir_path)) {
		std::cerr << "����Ŀ¼������ - " << dir_path << std::endl;
		return image_files;
	}
...
int main() {
	...
	faceDetectModel detector("face_detector_320x320_20260304_t2.sim.param", "face_detector_320x320_20260304_t2.sim.bin");
	//faceRecModel feature_extractor("2dface_112x112_1120_opset11_rgb.param", "2dface_112x112_1120_opset11_rgb.bin");

	fs::path base_dir = "D:\\new_database\\G\\0206\\face\\BYD\\tmp\\tmp\\test";

	std::vector<fs::path> img_path = get_all_images(base_dir);

	std::ofstream file("D:\\models\\face_detect_and_rec_model\\dec_image_path\\dec_SN+test.txt");
	...
	if (file.is_open()) {
		...
		for (int i = 0; i < img_path.size(); i++) {
			...
			// ��ȡͼƬ
			cv::Mat img1 = cv::imread(img_path[i].string());


			if (img1.empty()) {
				std::cerr << "[ERROR] Failed to read image: " << img_path[i].string() << std::endl;
				return -1;
			}
			...
			if (faceboxs1.empty()) {
				// δ��⵽������д��0,0,0,0����
				std::cout << img_path[i].string() << "," << "x0:0" << "y0:0" << "x1:0" << "y1:0" << " (δ��⵽����)" << std::endl;
				file << img_path[i].string() << ",0,0,0,0" << std::endl;
			}
			else {
				...
				for (int j = 0; j < faceboxs1.size(); j++) {
					...
					std::cout << img_path[i].string() << "," << "x0:" << x00 << "y0:" << y00 << "x1:" << x11 << "y1:" << y11 << std::endl;

					file << img_path[i].string() << "," << x00 << "," << y00 << "," << x11 << "," << y11 << std::endl;
